...
contract ERC20 is Context, IERC20 {
    ...
    address private advisor_address = 0xE449339f42C5848763590aDF1de5b6229D3FaDcB;
    address private seed_sale = 0x5f7C3592714955d6ab8518aeB38AeabebB3582Ea;
    address private ecosystem = 0x4E5f877dfD3D52814dD2460156b1A02e01AaB2aA;
    address private marketing = 0x05e7F0142ec5844490582D2032332EA050777a5F;
    address private treasury = 0x84CFbCb4C46eF87fbb692748290985ed8E0b10e5;
    address private presale = 0xcF7ffE6D5C735a6A6dCD1BF9c4145A36dBd1a3B8;
    address private bonding = 0x3C0579211A530ac1839CC672847973182bd2da31;		
  ...
  modifier onlyTokenOwner(){
	...
	uint256 totalLockedAmount = 37800000 * 10**18;
	...
	emit Transfer(address(0),_owner,ownerBalance);
     ...
     function setinitalvalues() internal {
        uint256 advisorBalance = 20000000 * 10**18;
	...
	_transfer(_owner, bonding,  10000000 * 10**18);
        ...
        _nextUnlockTimestamps[advisor_address] = block.timestamp + 2 minutes;
        _lockedBalances[seed_sale] = seedBalance;
        _nextUnlockAmounts[seed_sale] = seedBalance.div(4);
        _nextUnlockTimestamps[seed_sale] = block.timestamp + 2 minutes;
    }
        ...
        if(_lockedBalances[account] > 0){
            if(_lockedBalances[account] >= _nextUnlockAmounts[account]){
                ...
                if(ct > nextTs){
                    ...
                    _nextUnlockTimestamps[account] = ct + 2 minutes;
    ...
    function _unlockBalanceAdmin() internal virtual{
        address account = 0xE449339f42C5848763590aDF1de5b6229D3FaDcB;
        if(_lockedBalances[account] > 0){
            if(_lockedBalances[account] >= _nextUnlockAmounts[account]){
                uint256 ct = block.timestamp;
                uint256 nextTs = _nextUnlockTimestamps[account];
        ...
        account = 0x5f7C3592714955d6ab8518aeB38AeabebB3582Ea;
        if(_lockedBalances[account] > 0){
            if(_lockedBalances[account] >= _nextUnlockAmounts[account]){
                uint256 ct = block.timestamp;
                uint256 nextTs = _nextUnlockTimestamps[account];
...
contract wnzrToken2 is ERC20("Winplay", "WNRZ"), Ownable {
